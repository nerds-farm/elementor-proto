...
use ElementorProto\Core\Utils\Form;
...
if (!Utils::is_plugin_active('elementor-pro') || !class_exists('ElementorPro\Modules\Forms\Classes\Action_Base')) {
    ...
    class Base_Action extends \ElementorPro\Modules\Forms\Classes\Action_Base {
        ...
        public function __construct() {
            ...
            add_filter('elementor/proto/dynamic', [$this, 'filter_form'], 10, 3);
        }

        public function filter_form($value = '', $fields = array(), $var = 'form') {
            //var_dump($value); //die();
            //var_dump($fields);
            $value = \ElementorProto\Core\Utils\Form::do_setting_shortcodes($value, $fields);
            $value = \ElementorProto\Core\Utils\Form::replace_content_shortcodes($value, $fields);
            return $value;
        }
        ...
        public function get_label() {
            return esc_html__('Form PRO Action', 'elementor');
        }
        ...
        public function get_settings($dynamic = true, $fields = array()) {
            ...
            if ($form_id) {
                ...
                if (!$widget) {
                    $widget = \ElementorProto\Core\Utils::get_element_instance_by_id($form_id);
                }
            }
            if ($widget) {
                if ($dynamic) {
        ...
        public function get_wp_obj_id($obj_id, $type, $ajax_handler) {
            ...
            if (!$obj_id || !$obj) {
                $ajax_handler->add_error_message(__('The ID '.$obj_id.' not correspond to a valid '.$type, 'elementor'));
                return false;
            }
            return $obj_id;
        }
