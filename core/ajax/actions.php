...
namespace ElementorProto\Core\Ajax;
...
class Actions {
    ...
    public function get_control_options($params, $control_options = []) {
        ...
        $control_options = apply_filters('elementor/proto/e_query/options/' . $params['query_type'], $control_options, $params);
    ...
    public function get_control_search(array $params, $control_options = []) {
        ...
        $control_options = apply_filters('elementor/proto/e_query/search/' . $params['query_type'], $control_options, $params);
    ...
    public function get_posts($params, $control_options = []) {        
        ...
        switch ($object_type) {
            ...
            default:
                ...
                if (class_exists('ElementorProto\Overrides\E_Query')) {
                    $query = new \ElementorProto\Overrides\E_Query($query_params);
                } else {
    ...
    public function _get_posts($params, $control_options = []) {
        ...
        if ($is_txt) {            
            ...
            if (class_exists('ElementorProto\Overrides\E_Query')) {
                $the_query = new \ElementorProto\Overrides\E_Query($args);
            } else {
    ...
    public function get_acf($params, $control_options = []) {        
        ...
        $acfs = \ElementorProto\Core\Utils\Acf::get_acf_fields($types);
        if (!empty($acfs)) {
    ...
    public function get_jet($params, $control_options = []) {        
        $types = (!empty($params['object_type'])) ? $params['object_type'] : array();
        $jets = \ElementorProto\Core\Utils\Jet::get_jet_fields($types);
        if (!empty($jets)) {
    ...
    public function _get_jet($params, $control_options = []) {
        $uid = (array) $params['id'];
        foreach ($uid as $aid) {
            $title = \ElementorProto\Core\Utils\Jet::get_jet_field($aid);
            if (!empty($title)) {
