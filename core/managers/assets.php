...
final class Assets {
    ...
    public function __construct() {
        ...
        do_action('elementor/proto/assets');
    ...
    public function register_core_assets() {
        $assets_path = ELEMENTOR_PROTO_PATH . 'assets' . DIRECTORY_SEPARATOR;
        ...
        if (!\Elementor\Plugin::$instance->editor->is_edit_mode()) {
            if (!empty(self::$styles)) {
                foreach (self::$styles as $skey => $sstyle) {
                    ...
                    if ($sstyle) {
                        $style .= '<style id="' . $skey . '">' . $sstyle . '</style>';
                    }
                }
            }
        }
        ...
        if (!\Elementor\Plugin::$instance->editor->is_edit_mode()) {
            if (!empty(self::$scripts)) {
                foreach (self::$scripts as $skey => $sscript) {
                    ...
                    if ($sscript) {
                        $script .= '<script id="' . $skey . '">' . $sscript . '</script>';
                    }
                }
            }
        }
