...
final class Controls {
    ...
    public function get_controls() {
        $elements = array();
        $path = ELEMENTOR_PROTO_PATH . 'core' . DIRECTORY_SEPARATOR . 'controls' . DIRECTORY_SEPARATOR;
        if (is_dir($path)) {
    ...
    public function get_groups() {
        $elements = array();
        $path = ELEMENTOR_PROTO_PATH . 'core' . DIRECTORY_SEPARATOR . 'controls' . DIRECTORY_SEPARATOR. 'groups' . DIRECTORY_SEPARATOR;
        if (is_dir($path)) {
            $files = Utils::glob($path . '*.php');
            foreach ($files as $ele) {
                $file = basename($ele);
    ...
    public function init_controls() {
        ...
        \ElementorProto\Plugin::instance()->assets_manager->register_core_assets(); // TODO: check why this is needed!
        do_action('elementor/proto/controls');
    }
